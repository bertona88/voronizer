...
def _jump_flood_cpu(u, norm):
    ...
    coords = np.stack(indices, axis=-1).astype(np.float32)
    result = np.zeros(arr.shape + (4,), dtype=np.float32)
    result[..., :3] = coords
    result[..., 3] = distances.astype(np.float32)
    return result

def _sdf3d_cpu(u, norm):
    arr = np.asarray(u, dtype=np.float32)
    inside = arr <= 0
    outside = ~inside
    outside_dist = ndimage.distance_transform_edt(outside).astype(np.float32)
    inside_dist = ndimage.distance_transform_edt(inside).astype(np.float32)
    sdf = outside_dist
    sdf[inside] = -inside_dist[inside]
    return sdf

def _simplify_cpu(u):
    arr = np.asarray(u, dtype=np.float32)
    neg_mask = arr <= 0
    result = np.full(arr.shape, 0.01, dtype=np.float32)
    if not np.any(neg_mask):
        return result
    pos_neighbor = ndimage.maximum_filter(arr, size=3, mode="nearest") > 0
    neg_neighbor = ndimage.minimum_filter(arr, size=3, mode="nearest") < 0
    surface = neg_mask & pos_neighbor & neg_neighbor
    result[neg_mask] = -0.01
    result[surface] = 0.0
    return result
